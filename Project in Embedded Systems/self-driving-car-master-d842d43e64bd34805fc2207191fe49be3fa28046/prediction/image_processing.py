...
import CNNInterface as cnn_tensorflow
...
class ImageProcessing(ThreadingMixIn):
    def __init__(self):
 ...
 #       self.tf_sess, self.tf_graph = cnn_tensorflow.CNNpredict_init("world1-1")
    ...
    def image_read(self):
        def _image_queue(data_list):
            ...
            for _data in list_name:
                ...
                file_path = os.path.join(self.SRC_FOLDER, "img/%s" %image_name)
        ...
        pre_count = 0
        while pre_count < 10:
            name_list = os.listdir(self.SRC_FOLDER)
            ...
            for _file_name in name_list:
                file_path = os.path.join(self.SRC_FOLDER, _file_name)

                #file_path = os.path.join(self.SRC_FOLDER, PREDICT_FOLDER+"/"+_file_name)
                if not os.path.isfile(file_path) or imghdr.what(file_path) != "jpg":
                    pre_count = pre_count+1
                    continue
                pre_count = 0
                image = Image.open(file_path).convert("L")
    ...
    def image_execute_test(self, file_path):
        return  3

    def image_execute(self, file_path, sess, graph):
        #self.SRC_FOLDER = src
        #print("image processing starting.....", time.time())
        image = Image.open(file_path).convert("L")
        os.remove(file_path)
        box = (0, 26, 160, 120)
        area = image.crop(box)
        ...
        np_list = image_list.reshape(1,image_list.shape[0]*image_list.shape[1]).astype(int) //255
        max_idx = numpy.argmax(np_list)
        #print ("image processing end.... cnn starting.....", time.time())
        ret, poss = cnn_tensorflow.CNNpredict(np_list, sess, graph)
        #print ("cnn ending......", time.time())
        return ret[0]


        '''
        #self.DST_FLODER = dst
        self.CSV_FOLDER = os.path.join(src, "img/name_svr.csv")
        ...
        #while self.process_status in ["Prediction", "Training"] or not(self.queue_data.empty()):
            ...
            count = count + 1
            image_list = numpy.asarray(thresholdedData)
            ...
            #srv_value = "Teansorflow"
            ret, poss = cnn_tensorflow.CNNpredict(numpy.asarrry(_list)[None,:], "world1-1")
            return ret
            ...
            self.IMAGE_DATA_LIST.append(_list)
        '''
        #TODO: Donmin provide the callable function name -- Tensorflow
        '''
...
if __name__ == '__main__':
    ...
    print (time.time())
    name_list = os.listdir(src_file)
    counter = 0
    for _file_name in name_list:
        file_path = os.path.join(src_file, _file_name)
        print (file_path)
        root.image_execute(src_file, file_path, counter)
        counter = counter + 1
    print (time.time())
    '''
